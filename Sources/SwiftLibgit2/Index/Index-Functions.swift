...
/// - Parameter index: The index to free. The underlying type must be
/// `git_index`.
///
/// ## C Equivalent
///
...
///
/// If `force` is `false`, this function performs a "soft" read that reloads
/// the index data from disk only if it has changed since the last time it
...
/// Writes the given index from memory back to the disk, using an atomic
/// file lock.
/// - Parameter index: The index to write. The underlying type must be
/// `git_index`.
/// - Returns: A ``GitErrorCode`` instance.
///
/// ## C Equivalent
...
/// does not exist on-disk, it will be zeroed out.
///
...
/// - Parameters:
///   - index: The index to update. The underlying type must be `git_index`.
///   - dir: The path to the directory within which to remove all entries.
///   - stage: The stage to search.
/// - Returns: A ``GitErrorCode`` instance.
///
/// ## C Equivalent
...
/// - Parameters:
///   - out: The ``GitIndexEntry`` instance in which to store the next index
///   entry.
///   - iterator: The index iterator to use. The underlying type must be
///   `git_index_iterator`.
/// - Returns: A ``GitErrorCode`` instance.
///
/// ## C Equivalent
...
/// - Parameter iterator: The index iterator to free. The underlying type must
/// be `git_index_iterator`.
///
/// ## C Equivalent
///
...
/// entry represents the position of the blob relative to the repository's
/// root folder.
///
/// If a previous index entry exists that has the same path as the given
/// index entry, it will be replaced.
///
/// This function forces the file to be added to the given index, regardless
/// of ignore rules. If the file is the result of a merge conflict, it will
/// no longer be marked as conflicting. The data about the conflict will be
...
/// If a file is already tracked in the index, then it will be updated even if
/// it is ignored. Use the ``GitIndexAddOptionT/gitIndexAddForce`` flag to skip
...
///
/// To emulate `git add -A` with the `dry-run` option, use a callback function
/// that always returns a positive value.
///
...
/// Removes the index entries matching the specified files in the working
/// directory.
/// - Parameters:
///   - index: The index to update. The underlying type must be `git_index`.
///   - pathspec: The path patterns to use.
...
/// file no longer exists, or otherwise updating the information (including
...
/// Finds the first position of any entries matching the given path in the
/// given index.
/// - Parameters:
...
/// Gets the index entries that represent a conflict of the specified file in
/// the given index.
/// - Parameters:
///   - ancestorOut: The ``GitIndexEntry`` instance in which to store the
///   ancestory entry.
///   - ourOut: The ``GitIndexEntry`` instance in which to store "our" entry.
///   - theirOut: The ``GitIndexEntry`` instance in which to store "their"
///   entry.
///   - index: The index to search. The underlying type must be `git_index`.
...
/// Removes the index entries that represent a conflict of the specified file
/// in the given index.
/// - Parameters:
///   - index: The index to update. The underlying type must be `git_index`.
///   - path: The path to the file from which to remove conflicts.
/// - Returns: A ``GitErrorCode`` instance.
///
/// ## C Equivalent
///
...
/// - Parameter index: The index to update. The underlying type must be
/// `git_index`.
/// - Returns: A ``GitErrorCode`` instance.
///
/// ## C Equivalent
...
/// - Parameter index: The index to check. The underlying type must be
/// `git_index`.
/// - Returns: Whether the given index contains entries representing file
/// conflicts.
///
/// ## C Equivalent
///
...
/// - Parameters:
///   - iteratorOut: The pointer in which to store the index conflict iterator.
///   The underlying type must be `git_index_conflict_iterator`.
...
///   - theirOut: The ``GitIndexEntry`` instance in which to store "their"
///   entry.
///   - iterator: The index conflict iterator to use. The underlying type must
///   be `git_index_conflict_iterator`.
/// - Returns: A ``GitErrorCode`` instance.
///
/// ## C Equivalent
///
...
/// Frees the memory allocated for the given `git_index_conflict_iterator`
/// instance.
/// - Parameter iterator: The index conflict iterator to free. The underlying
/// type must be `git_index_conflict_iterator`.
///
/// ## C Equivalent
///
