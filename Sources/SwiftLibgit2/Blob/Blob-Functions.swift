...
{
    guard let blobOID: UnsafePointer<git_oid> = git_blob_id(blob)
    else
    {
        return nil
    }
...
{
    let isBinary: Int32 = git_blob_is_binary(blob)
    
    return Bool(isBinary)
}
...
) -> Bool?
{
    ...
    return try? data.withCBuffer
    {
        ...
        let isBinary: Int32 = git_blob_data_is_binary(
            cData,
            cDataCount
        )
